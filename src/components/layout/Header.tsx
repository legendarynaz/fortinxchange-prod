...
const Header: React.FC<HeaderProps> = ({ selectedMarket, onMarketChange, onNavigate, onLogout, currentView, isKycVerified }) => {
  ...
  return (
    <header className="bg-white/80 backdrop-blur-lg border-b border-slate-200 h-16 flex items-center px-4 md:px-6 sticky top-0 z-40">
      <div className="flex items-center space-x-4">
        <LogoIcon />
        <h1 className="text-xl font-bold text-slate-900 tracking-tighter">FortinXchange</h1>
      </div>
      ...
      {currentView === 'trade' && (
        <div className="ml-6 border-l border-slate-200 pl-6">
          <div className="relative" ref={marketDropdownRef}>
            ...
            >
              <span className="font-mono text-slate-900 font-bold text-lg">{selectedMarket.name}</span>
              <ChevronDownIcon className={`w-5 h-5 text-slate-500 transition-transform ${isMarketOpen ? 'rotate-180' : ''}`} />
            </button>
            {isMarketOpen && (
              <div className="absolute top-full mt-2 w-48 bg-white border border-slate-200 rounded-md shadow-lg py-1 z-50">
                {MARKETS.map(market => (
                  <button
                    key={market.id}
                    onClick={() => handleSelectMarket(market)}
                    className="w-full text-left px-4 py-2 text-sm text-slate-800 hover:bg-sky-100"
                  >
                    {market.name}
                  </button>
                ))}
      ...
      <div className="flex items-center space-x-6">
        ...
        <div className="relative" ref={userDropdownRef}>
            <button onClick={() => setUserOpen(!isUserOpen)} className="w-8 h-8 rounded-full bg-sky-200 flex items-center justify-center text-sky-700 font-bold hover:bg-sky-300">
                U
            </button>
            {isUserOpen && (
                 <div className="absolute top-full right-0 mt-2 w-56 bg-white border border-slate-200 rounded-md shadow-lg py-1 z-50">
                    <div className="px-4 py-2 border-b border-slate-200">
                        <p className="text-sm font-medium text-slate-900 truncate">user@email.com</p>
                        {isKycVerified ? (
